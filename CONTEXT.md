...
- **markdown** (`gmaps-shared/src/markdown.ts`) — the shared LLM-markdown renderer (`mdInline`/`mdToHtml`): pure string→html, deliberately hand-rolled (not a library) to tame the malformed markdown models emit — stray/odd `**`, `__bold__`, snake_case `_` — where a spec parser would leave literal markers or need XSS sanitizing. Both packages consume it and keep their own DOM sink (`renderMarkdown`: extension `innerHTML`, web `createContextualFragment`). Tested in `markdown.test.ts`.
- **Score grid** (`extension/src/shared/score-grid.ts`) — `setupScoreGrid`: the shared PLP behaviour the six product-grid scrapers used to each hand-roll — badge every card with its net score, then re-rank the container by `data-nps` (scored desc, unscored last), progressively as scores land. Ranking is CSS-order-only by default (`orderByCssBand`) so re-rendering hosts never fight it; `data-nps` rides on the badge so rank survives wrapper recreation. Owns the rAF coalescing and the debounced `MutationObserver`; the per-site parts are injected slots (`scoreForCard`, `placeBadge`, `discover`, `applyOrder`) with batteries-included helpers (`structuralContainers` / `containersBySelector`; `orderByCssBand` / `orderByCssImportant` / opt-in `orderByAppend`). Each ranking also tints its **best-ratio picks** (`markBestRatios`: a displayed ratio ≥2 points above every card ranked higher, score ≥20) and `]`/`[` cycle them (`cycleBestRatios`) — both shared with Amazon search.
