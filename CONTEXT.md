...
- **Search** — arbitrary review-text search within a place.
...
- **Review-collection loop** (`gmaps-shared/collect.ts`) — `collectPaged`/`collectSort`/`collectToken`: paginate a cursor URL → parse → dedup by `reviewId` → stop (no cursor / maxPages / `onPage` 'stop' / stabilized / aborted). The kernel both packages used to duplicate.
